...
def plot_path(args):
    ...
    # ROS map image files are stored top-row first. RViz/map_server display them
    # with the YAML origin as the lower-left world origin, so use origin='upper'
    # when drawing the raw image over the same world extent.
    ax.imshow(image, cmap="gray", origin=args.image_origin, extent=extent)
